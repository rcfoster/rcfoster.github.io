...
for(sim in 1:n_sims) {
  ...
  # For gamma distribution the following transformations are used:
  #
  # mu = alpha/beta
  # M = beta
  #
  ...
  theta <- rgamma(n_subjects, true_mu*true_M, true_M)
  
  # Create scored response matrix and simulate scores for each subject using a Poisson distribution
  ...
  kr.20[sim] = (1 - (sum(apply(y,2,mean))/var(x)))
  kr.21[sim] = (1 - mean(x)/var(x))
