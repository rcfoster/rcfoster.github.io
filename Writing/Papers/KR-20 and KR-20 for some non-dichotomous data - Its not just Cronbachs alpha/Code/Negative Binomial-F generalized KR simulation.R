...
# code for the Negative Binomial - F model
...
# Not all values of true_mu are possible for a given set of (rho, n_items). 
# It must be set bigger than 1. When errors occur, it is suggested to move
# true_mu closer to, but larger than, 1.
...
for(sim in 1:n_sims) {
  ...
  # A complicated transformation is used for the F distribution. 
  ...
  # Create scored response matrix and simulate scores for each subject using a geometric
  # distribution. The geometric distribution in R has mean theta = (1-p)/p, so the 
  # transformation p = 1/(1 + theta) is used.
