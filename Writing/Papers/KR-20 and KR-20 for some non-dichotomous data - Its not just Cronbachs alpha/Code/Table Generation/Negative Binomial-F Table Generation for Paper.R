# This code is to generate Table 4, simulation results for the negative binomial - F model, in the paper. 
# It is essentially the file "Negative Binomial-F generalized KR simulation.R" but without the 
...
for(par in 1:dim(pars)[1]) {
  ...
  for(sim in 1:n_sims) {
    ...
    d1 = -2*true_mu*(true_M-1)/((true_mu*(true_mu-1)*true_M-2))
    d2 = 2*true_mu/(true_mu-1)
    ...
    kr.20[sim] = n_items/(n_items + 1)*(1 - (sum(apply(y,2,mean) + apply(y,2,mean)^2)/var(x)))
    kr.21[sim] = n_items/(n_items + 1)*(1 - n_items*(mean(x/n_items) + mean(x/n_items)^2)/var(x))
  ...
  alpha.rmse = format(round(sqrt(mean((alpha - true_rho)^2,na.rm=T)),3),nsmall = 3)
  alpha.bias = format(round(mean(alpha - true_rho,na.rm=T),3), nsmall = 3)
  alpha.sd = format(round(sd(alpha,na.rm=T),3), nsmall = 3)
  
  kr.20.rmse = format(round(sqrt(mean((kr.20 - true_rho)^2,na.rm=T)),3),nsmall = 3)
  kr.20.bias = format(round(mean(kr.20 - true_rho,na.rm=T),3),nsmall = 3)
  kr.20.sd = format(round(sd(kr.20,na.rm=T),3),nsmall = 3)
  
  kr.21.rmse = format(round(sqrt(mean((kr.21 - true_rho)^2,na.rm=T)),3),nsmall = 3)
  kr.21.bias = format(round(mean(kr.21 - true_rho,na.rm=T),3),nsmall = 3)
  kr.21.sd = format(round(sd(kr.21,na.rm=T),3),nsmall = 3)
